...
- 支持对当前 attached 本地分支的 `HEAD` 执行安全 Amend：Preview/Confirm token 绑定分支、HEAD、原 author/parents、提交信息、阻塞引用、完整 index 与 staged binary diff；本地已知远端引用包含 HEAD 或标签精确指向 HEAD 时拒绝，Detached/unborn/冲突及 merge、rebase、cherry-pick、revert 进行中均拒绝；执行使用 `write-tree`、`commit-tree` 与 expected-old-OID `update-ref`，允许仅修改信息且不执行 commit hooks、编辑器或本次签名，详细边界见 `docs/adr/0024-safe-head-amend.md`；
...
- 支持针对已配置远端的 fetch、当前分支 upstream-only 的安全 Pull 和非强制 Push；当前本地分支没有 upstream 时，可将其 create-only 发布为远端新分支并设置 upstream；提交菜单中的“提交和推送”可在提交前选择一个已读取的远端分支，或输入要新建的远端分支，并在成功后设置 upstream；这些网络操作都提供 operation ID、结构化进度、显式取消、5 分钟总超时和进程树清理；Pull 只执行 fetch + `merge --ff-only`，已有 upstream 的 Push 只推送当前分支，分叉或非快进时拒绝操作；
